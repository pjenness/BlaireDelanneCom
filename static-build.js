...
async function createStaticApiData() {
  try {
    ...
    for (const endpoint of apiEndpoints) {
      ...
      // Calculate file path based on endpoint
      let filePath;
      if (endpoint === '/api/posts/featured/main') {
        filePath = path.join(BUILD_DIR, 'api/posts/featured/main');
      } else {
        filePath = path.join(BUILD_DIR, endpoint);
      }
      
      await writeFile(filePath, JSON.stringify(data));
      console.log(`Created static data for ${endpoint}`);
    }
  } catch (error) {
